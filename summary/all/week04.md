...
- Each internal node applies a binary test on a single feature: `X_j <= s` (a split).
...
- We partition the p-dimensional feature space into J disjoint regions R_1, R_2, ..., R_J.
- In each region R_j, the prediction is the mean of the training responses in that region.

### Prediction Formula
- **Regression tree prediction**: `f(x) = sum_{j=1}^{J} c_j * I(x in R_j)`
  - where `c_j = mean(y_i : x_i in R_j)` (the average response in region j)
  - `I(x in R_j)` = 1 if x falls in region j, 0 otherwise
...
- **RSS split criterion**: `RSS = sum_{i: x_i in R_1(j,s)} (y_i - c_1)^2 + sum_{i: x_i in R_2(j,s)} (y_i - c_2)^2`
  - where `c_1 = mean(y_i : x_i in R_1)`, `c_2 = mean(y_i : x_i in R_2)`
...
6. Stop when a stopping criterion is met (e.g., minimum node size n_min, maximum depth).
...
- **Misclassification rate**: `E = 1 - max_k(p_mk)`
  - where `p_mk = (1/N_m) * sum_{x_i in R_m} I(y_i = k)` = proportion of class k in region m
...
- **Gini index**: `G = sum_{k=1}^{K} p_mk(1 - p_mk)`
  ...
  - Can be written as: `G = 1 - sum_k p_mk^2`

#### Cross-Entropy / Deviance
- **Cross-entropy**: `D = -sum_{k=1}^{K} p_mk * log(p_mk)`
...
- **Impurity reduction**: `Delta_I = I(parent) - [N_L/N * I(left) + N_R/N * I(right)]`
  - where N_L, N_R = number of observations in left/right child
  - I() = chosen impurity measure (Gini or cross-entropy)

### Prediction in leaves
- In each leaf, predict the majority class: `k* = argmax_k p_mk`
- Probabilities can also be read off: `P(class=k | x in R_m) = p_mk`
...
- Stop splitting when node size < n_min (minimum node size).
...
- **Cost-complexity criterion**: `C_alpha(T) = sum_{m=1}^{|T|} N_m * Q_m(T) + alpha * |T|`
  - `|T|` = number of terminal nodes (leaves)
  - `Q_m(T)` = node impurity (e.g., RSS/N_m for regression, Gini for classification)
  - `alpha >= 0` = complexity penalty (tuning parameter)
  - When alpha = 0: full tree is selected (no penalty).
  - As alpha increases: smaller trees are preferred.
- For each alpha, there is a unique smallest subtree T_alpha that minimises C_alpha(T).
- A sequence of nested trees is produced as alpha increases: T_0 > T_1 > T_2 > ... > T_root.

### Selecting alpha via Cross-Validation
1. Grow full tree T_0 on training data.
2. For each value of alpha in the sequence:
   - Obtain the corresponding pruned subtree T_alpha.
...
2. **Grow**: Recursively apply binary splits minimising RSS until n_min is reached. (No pruning yet.)
3. **Prune**: Apply cost-complexity pruning to generate sequence of subtrees T_alpha.
...
- At each node, scanning all features and split points: O(N * p) per node.
- Total tree growth: O(N * p * log N) approximately.
...
- **Variable importance for feature j**: `VI_j = sum_{splits using j} N_t * Delta_I_t`
  - where N_t = number of observations at node t, Delta_I_t = impurity reduction at node t
- Higher VI means the variable contributes more to prediction.

### Reading a Tree
- Each internal node shows the split rule (e.g., X_1 <= 0.5).
- Left branch = TRUE (X_1 <= 0.5), Right branch = FALSE.
...
- Typical minimum node size: n_min = 5 (regression), n_min = 1 (classification)
