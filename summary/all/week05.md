...
- The probability estimate for class k in region m: `p_mk = proportion of class-k observations in region m`.
...
|---------|---------|-----------|
| Misclassification rate | `E = 1 - max_k(p_mk)` | Not sensitive to probability shifts; used for pruning |
| Gini index | `G = sum_k p_mk(1 - p_mk)` | Sensitive to probability shifts; preferred for growing |
| Cross-entropy | `D = -sum_k p_mk log(p_mk)` | Sensitive to probability shifts; preferred for growing |
...
- For binary classification (K=2), with p = p_m1:
...
- The prediction in each leaf is the class with the highest proportion (argmax_k p_mk).
...
### Key Concepts
- After growing a full tree T_0 (until n_min is reached), we prune using cost-complexity.
- **Cost-complexity criterion**: `C_alpha(T) = R(T) + alpha * |T|`
  - `R(T)` = total misclassification cost (or RSS for regression) = `sum_{m=1}^{|T|} N_m * Q_m`
...
- As alpha increases from 0 to infinity, we get a sequence of nested subtrees: T_0 > T_1 > ... > root.
...
2. Find the sequence of subtrees T_0 > T_1 > ... via weakest link pruning.
...
- On average, each bootstrap sample contains approximately 63.2% of the unique original observations.
  - P(observation i NOT in bootstrap sample) = (1 - 1/N)^N → 1/e ≈ 0.368 as N → infinity
...
2. **Regression prediction**: `f_bag(x) = (1/B) * sum_{b=1}^{B} T_b(x)`
3. **Classification prediction**: majority vote of {T_1(x), T_2(x), ..., T_B(x)}

### Bagging Bias
- The bias of the bagged estimator equals the bias of any individual tree (since all trees are identically distributed):
- **Bagging bias**: `E(y_hat - y) = E[(1/B) * sum_{b=1}^{B} (y_hat_b - y)] = (1/B) * sum_b E(y_hat_b - y) = E(y_hat_b - y)`
...
- **Bagging variance**: `Var(y_hat) = rho * sigma^2 + (1-rho)/B * sigma^2`
  ...
  - Second term `(1-rho)/B * sigma^2` goes to zero as B → infinity.
- As B → infinity: Var → rho * sigma^2.
- The limiting factor for variance reduction is rho (the correlation between trees).
- If rho = 0 (trees independent): Var = sigma^2 / B (full variance reduction).
- If rho = 1 (all trees identical): Var = sigma^2 (no variance reduction).

### Why Trees are Correlated in Bagging
- All trees are trained on slightly different versions of the same data.
- If one feature is very strong (dominant), all trees will use it near the root → high correlation.
...
- In practice, B ~ 100-500 is usually sufficient; error stabilises.
