...
- `E(y_hat - y) = E[(1/B) sum_{b=1}^B (y_hat_b - y)] = (1/B) sum_b E(y_hat_b - y) = E(y_hat_b - y)`
...
2. For b = 1 to B:
   ...
   b. Grow a tree by repeating until minimum node size n_min is reached (DO NOT prune):
...
- **Regression**: `y_hat = (1/B) * sum_{b=1}^B T_b(x)`
...
- Default heuristics (rules of thumb):
  - **Classification**: `m = floor(sqrt(p))`
  - **Regression**: `m = floor(p/3)`
...
- Both bagging/RF and ridge regression handle p >> n (more variables than observations) problems.
...
- **Gini importance for j**: `VI_j^Gini = sum_{trees} sum_{splits on j} N_t * Delta_Gini_t`
...
### Key Concepts
- An n x n proximity matrix P is built from the random forest.
...
- The final classifier is: `G(x) = sign[sum_{m=1}^M alpha_m * G_m(x)]`
...
1. Initialise observation weights: `w_i = 1/N` for i = 1, 2, ..., N.
2. For m = 1 to M:
   a. Fit classifier G_m(x) to training data using weights w_i.
   b. Compute weighted error of G_m:
      - `err_m = sum_{i=1}^N w_i * I(y_i != G_m(x_i)) / sum_{i=1}^N w_i`
...
- If err_m = 0.5 (random): alpha_m = 0 (no contribution).
- If err_m = 0 (perfect): alpha_m → infinity.
- If err_m > 0.5 (worse than random): alpha_m < 0 (classifier is negated).
- The weights alpha_m weight better classifiers more heavily in the final vote.
- Shrinkage (learning rate): contribution of each tree can be scaled by factor 0 < nu < 1 to slow down learning and improve generalisation.

### Shrinkage in Boosting (Learning Rate)
- `F_m(x) = F_{m-1}(x) + nu * f_m(x)` where 0 < nu < 1 is the learning rate (shrinkage).
- Smaller nu: each tree contributes less; need more trees M to converge; better generalisation.
...
- **Forward Stagewise Algorithm (Regression)**:
  ...
  2. Repeat M times:
     ...
     c. Set f_m(x) = epsilon * g(x) (shrink with epsilon).
...
- AdaBoost builds an additive logistic regression model: `F(x) = log[P(y=1|x) / P(y=-1|x)]`
...
### Comparison of Loss Functions (for classification, y*f is the margin)
| Loss | Formula | Properties |
|------|---------|-----------|
| Misclassification | I(y != sign(f)) | Not differentiable; not usable in gradient methods |
| Exponential | exp(-y*f) | Fast computation; sensitive to mislabelled data |
...
| Support Vector | max(0, 1 - y*f) | Sparse; used in SVMs |
...
1. Initialise: `F_0(x) = argmin_gamma sum_{i=1}^N L(y_i, gamma)` and m = 0.
2. For m = 1 to M:
   a. For i = 1 to N, compute negative gradient (pseudo-residuals):
      - `r_{im} = -[dL(y_i, F(x_i)) / dF(x_i)]_{F=F_{m-1}}`
   b. Fit a regression tree to r_{im}, giving terminal regions R_{jm}, j=1,...,J_m.
...
- If loss = absolute error: negative gradient = sign(y_i - F_{m-1}(x_i)). More robust to outliers.
...
- ANOVA expansion: `eta(X) = sum_j eta_j(X_j) + sum_{jk} eta_{jk}(X_j, X_k) + sum_{jkl} eta_{jkl}(X_j, X_k, X_l) + ...`
...
- Loss: `-sum_{k=1}^K y_k * log(P(y_k = 1 | x))`
- Score function per class k: F_k, k=1,...,K (initialised as zeros).
- Class probabilities via softmax: `P_k(y=1|x) = e^{F_k(x)} / sum_{k'=1}^K e^{F_{k'}(x)}`
