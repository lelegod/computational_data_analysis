...
- $X_i \sim \mathcal{N}(\mu_j, \Sigma_j)$ if latent variable $Z_i = j$.
- Mixing proportions $\pi_j = P(Z_i = j)$; must sum to 1: $\sum_j \pi_j = 1$.
- Parameters: $\theta = (\pi_1,\ldots,\pi_K, \mu_1,\ldots,\mu_K, \Sigma_1,\ldots,\Sigma_K)$.
- Solved by the **EM algorithm** (alternates E-step and M-step until convergence).
- **E-step:** computes posterior probability $\gamma_{ij} = P(Z_i=j \mid x_i)$ using Bayes' rule.
- **M-step:** updates parameters as weighted means/covariances using $\gamma_{ij}$ as weights.
- $\pi_j^{(\text{new})} = (1/n) \sum_i \gamma_{ij}$ (average of soft assignments).
- K-means is a special case of GMM with hard assignments and equal spherical covariances.
- High-dimensional GMM tricks: shared covariance, diagonal covariance, regularize $\Sigma = \Sigma + \lambda I$, use PCA first.
...
- Choose $K^* = \arg\min_k \{K \mid G(K) \geq G(K+1) - s'_{K+1}\}$ where $s'_{K+1} = \text{std}(\log(U_K))\cdot\sqrt{1+1/20}$.
...
|---------|------------|-------------|
| $\min \sum_k \sum_{i \in C_k} \|x_i - \mu_k\|^2$ | K-means objective | Defining K-means |
| $W_k = \sum_\ell \frac{1}{2N_l} D_\ell$ | Within-cluster dissimilarity | Gap statistic |
| $D_\ell = N_l \sum_{i \in C_l} \|x_i - \bar{x}_l\|^2$ | Cluster sum of squares | Computing $W_k$ |
| $G(K) = \log(U_k) - \log(W_k)$ | Gap statistic | Choosing $K$ |
| $K^* = \arg\min_k \{K \mid G(K) \geq G(K+1) - s'_{K+1}\}$ | Gap selection rule | Finding optimal $K$ |
| $s'_{K+1} = \text{std}(\log(U_K))\cdot\sqrt{1+1/20}$ | Gap uncertainty | Gap selection rule |
| $s(i) = (b(i)-a(i))/\max\{a(i),b(i)\}$ | Silhouette score | Cluster quality |
| $\gamma_{ij} = \pi_j \mathcal{N}(x_i;\mu_j,\Sigma_j) / \sum_{j'}\pi_{j'}\mathcal{N}(x_i;\mu_{j'},\Sigma_{j'})$ | GMM E-step (soft assignments) | EM algorithm |
| $\mu_j = \sum_i\gamma_{ij}x_i / \sum_i\gamma_{ij}$ | GMM M-step mean update | EM algorithm |
| $\Sigma_j = \sum_i\gamma_{ij}(x_i-\mu_j)(x_i-\mu_j)^T / \sum_i\gamma_{ij}$ | GMM M-step covariance update | EM algorithm |
| $\pi_j = (1/n) \sum_i \gamma_{ij}$ | GMM M-step mixing proportion | EM algorithm |
| $\sum_j \pi_j = 1$ | Mixing proportions sum to 1 | GMM constraint |
...
- **❌ GMM gives hard cluster assignments** → ✓ GMM gives soft (probabilistic) assignments $\gamma_{ij} \in [0,1]$.
- **❌ In GMM, we observe $Z_i$ (cluster labels)** → ✓ $Z_i$ are **latent (unobserved)** variables; that's why we need EM.
- **❌ $\pi_j$ in GMM is the estimated mean of cluster $j$** → ✓ $\pi_j$ is the **mixing proportion** (prior probability that an obs belongs to cluster $j$).
- **❌ The E-step in EM updates the parameters** → ✓ The E-step computes soft assignments $\gamma_{ij}$; the **M-step updates parameters**.
...
- If gap statistic $G(K)$ first satisfies $G(K) \geq G(K+1) - s'_{K+1}$ → that is $K^*$.
