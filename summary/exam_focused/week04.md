...
- The result is a sequence of nested subtrees T_0 > T_1 > ... > T_root.
...
|---------|-----------|-------------|
| `c_j = mean(y_i : x_i in R_j)` | Leaf prediction (regression) | Predict in regression leaf |
| `RSS = sum (y_i - c_1)^2 + sum (y_i - c_2)^2` | Split criterion for regression | Choosing best split in regression tree |
| `p_mk = (1/N_m) sum I(y_i = k)` | Class proportion in leaf m | Computing Gini, entropy, error |
| `G = sum_k p_mk(1 - p_mk)` | Gini index | Split criterion for classification |
| `D = -sum_k p_mk * log(p_mk)` | Cross-entropy / deviance | Alternative split criterion |
| `E = 1 - max_k(p_mk)` | Misclassification rate | Pruning evaluation |
| `C_alpha(T) = sum_m N_m Q_m(T) + alpha * |T|` | Cost-complexity criterion | Pruning: balance fit vs. tree size |
| `Delta_I = I(parent) - [N_L/N * I(left) + N_R/N * I(right)]` | Impurity reduction at a split | Choosing best split (classification) |
| `VI_j = sum_{splits on j} N_t * Delta_I_t` | Variable importance for feature j | Interpreting which features matter |
...
- If n_min is large: shallower tree, more bias, less variance.
