...
- P(observation NOT in bootstrap) → 1/e ≈ 0.368 as N → infinity.
...
|---------|-----------|-------------|
| `p_mk = (1/N_m) sum I(y_i = k)` | Class proportion in leaf m | Compute impurity measures |
| `G = sum_k p_mk(1 - p_mk)` | Gini index | Growing classification tree |
| `D = -sum_k p_mk log(p_mk)` | Cross-entropy | Growing classification tree |
| `E = 1 - max_k(p_mk)` | Misclassification rate | Pruning/evaluation |
| `C_alpha(T) = R(T) + alpha * |T|` | Cost-complexity (pruning) | Select tree size |
| `f_bag(x) = (1/B) sum_{b=1}^B T_b(x)` | Bagging regression prediction | Predict with bagged tree |
| `E(y_hat - y) = E(y_hat_b - y)` | Bagging bias = individual tree bias | Understand that bagging doesn't reduce bias |
| `Var = rho*sigma^2 + (1-rho)/B * sigma^2` | Bagging variance | Understand variance reduction limits |
| `P(OOB) = (1 - 1/N)^N → 1/e ≈ 0.368` | OOB probability | Understand OOB sample size |
...
- If B → infinity: variance → rho * sigma^2 (not zero).
- If rho = 0 (impossible in pure bagging but theoretical): variance → sigma^2 / B.
