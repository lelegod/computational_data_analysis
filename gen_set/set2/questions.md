...
A. The irreducible error σ² can be reduced by choosing a sufficiently complex model.
B. EPE = Bias²(f̂) + Var(f̂) + σ², where the expectation is taken over both new observations y and the training set D.
C. As model complexity increases, training error decreases monotonically while test error forms a U-shape.
D. The variance component E[(f̂ − E[f̂])²] measures how far the average prediction is from the true function.
E. None of the above.

---

**Question (2)** [Week 1]
For a linear smoother Ŷ = SY, the effective degrees of freedom is defined as df(S) = trace(S). Which of the following are correct?

A. For OLS regression with p predictors, df = p.
B. For Ridge regression, as λ → ∞ the effective degrees of freedom approaches p.
C. For Ridge regression, as λ → 0 the effective degrees of freedom approaches p.
D. AIC with d = df(λ) for Ridge regression is asymptotically equivalent to leave-one-out cross-validation.
E. None of the above.
...
D. When p > N, OLS can be computed but has high variance because (X^T X) is near-singular.
E. None of the above.
...
D. Cost-complexity pruning generates a sequence of nested subtrees by increasing the complexity penalty α.
E. None of the above.

---

**Question (6)** [Week 5]
The variance of the bagged ensemble estimator (with B trees, pairwise correlation ρ, and individual tree variance σ²) is:

Var(ŷ_bag) = ρσ² + ((1−ρ)/B)σ²

Which of the following conclusions follow directly from this formula?

A. As B → ∞, the ensemble variance approaches zero regardless of ρ.
B. If all trees are perfectly correlated (ρ = 1), bagging provides no variance reduction.
...
A. The classifier weight αₘ = log[(1 − errₘ)/errₘ] is negative when errₘ > 0.5, effectively negating the weak classifier.
B. At each iteration, the weights of misclassified observations are decreased so future classifiers focus on easy cases.
C. AdaBoost is equivalent to forward stagewise additive modelling with exponential loss L(y, f) = exp(−y·f(x)).
...
min_{β,β₀} (1/2)‖β‖²   subject to   yᵢ(xᵢᵀβ + β₀) ≥ 1 ∀i

Which of the following are correct?

A. The margin width in the canonical formulation equals 1/‖β‖.
B. A point xᵢ is a support vector if and only if its Lagrange multiplier αᵢ > 0.
C. In the dual formulation, the training data appear only as pairwise dot products ⟨xᵢ, xⱼ⟩, enabling the kernel trick.
...
A. The PLS weight for feature j in component m is φ̂ₘⱼ = xⱼ^(m−1)ᵀ y, assigning higher weight to features correlated with y.
B. PLS latent components z₁, z₂, ..., zₘ are mutually uncorrelated (orthogonal).
...
**Question (13)** [Week 10]
In training a multi-layer perceptron (MLP), the backpropagation algorithm uses the chain rule. The weight gradient for layer ℓ is:

∂L/∂W^(ℓ) = δ^(ℓ) × (a^(ℓ−1))ᵀ

where δ^(ℓ) = (W^(ℓ+1))ᵀ δ^(ℓ+1) ⊙ σ'(z^(ℓ)). Which of the following are correct?

A. σ'(z) = σ(z)(1 − σ(z)) for the sigmoid activation, which is computationally convenient because σ(z) is already computed in the forward pass.
...
C. The gradient ∂L/∂W^(ℓ) requires only the error signal at layer ℓ and the input activation from the previous layer — not any future layer's weights.
...
**Question (15)** [Week 11]
Regarding Non-negative Matrix Factorization (NMF) with X ≈ WH, which statements are correct?
...
C. The Lee & Seung multiplicative update rule H_{kj} ← H_{kj} · (WᵀX)_{kj} / (WᵀWH)_{kj} guarantees non-negativity throughout optimization if initialized positively.
D. NMF solutions are generally not unique — for any invertible Q with WQ⁻¹ ≥ 0 and QH ≥ 0, (WQ⁻¹)(QH) is an equally valid factorization.
E. None of the above.
...
**Question (17)** [Week 12]
For a 3-way tensor X^{I × J × K} decomposed using the PARAFAC model:

x_{ijk} ≈ Σ_{r=1}^R a_{ir} b_{jr} c_{kr}

which of the following are correct?

A. PARAFAC is a special case of Tucker3 where the core tensor G is super-diagonal (identity-like tensor I^{R×R×R}).
...
D. In the PARAFAC ALS update, the mode-1 loading matrix is updated as A ← X_{(1)} (C ⊙ B)(C^TC * B^TB)^{-1}, where ⊙ is the Khatri-Rao product.
E. None of the above.
...
FMS = Σ_{r=1}^R (a_rᵀâ_r)/(‖a_r‖‖â_r‖) · (b_rᵀb̂_r)/(‖b_r‖‖b̂_r‖) · (c_rᵀĉ_r)/(‖c_r‖‖ĉ_r‖)

Which statements are correct?

A. If FMS ≈ R, the model is stable — both data halves recover the same components.
B. FMS is bounded between 0 and R, where R is the number of PARAFAC components.
C. A low FMS (FMS << R) suggests R is too small and more components should be added.
...
A researcher tunes a Ridge regression regularization parameter λ using 10-fold cross-validation and then reports the minimum CV error as the model's expected generalization error. Which of the following are correct?
...
C. The 1-SE rule would select a smaller λ than the minimum-CV-error rule, producing a more complex model.
...
B. When p >> n, the within-group covariance matrix Σ_XX is singular, making standard CCA ill-posed. Regularized CCA or Sparse CCA addresses this by adding a ridge penalty or L₁ sparsity.
C. PLS and CCA have identical objectives: both maximize Cov(Xu, Yv) subject to unit-variance constraints.
...
A pharmaceutical company has measured the transcriptome (gene expression) of n = 120 cancer cell lines across p = 8,000 genes (X ∈ R^{120 × 8000}). They have no class labels. They want to discover latent biological structure in the data.
...
max_{α}  Σᵢ αᵢ − (1/2) ΣᵢΣⱼ αᵢαⱼyᵢyⱼ⟨xᵢ, xⱼ⟩

subject to  αᵢ ≥ 0,  Σᵢ αᵢyᵢ = 0

**(a)** [2 points] State the primal SVM optimization problem and explain the canonical scaling convention. What is the geometric interpretation of the margin width in terms of ‖β‖?

**(b)** [3 points] Derive the dual problem from the primal using the Lagrangian L_P = (1/2)‖β‖² − Σᵢ αᵢ[yᵢ(xᵢᵀβ + β₀) − 1]. Show the stationarity conditions ∂L_P/∂β = 0 and ∂L_P/∂β₀ = 0 and use them to obtain the dual objective.

**(c)** [3 points] Explain the KKT complementary slackness condition αᵢ[yᵢ(xᵢᵀβ + β₀) − 1] = 0 and use it to explain why the SVM achieves informational sparsity. What distinguishes a support vector from a "safe" point?

**(d)** [2 points] The data are not linearly separable in the original feature space R^d. Explain the kernel trick: how does replacing ⟨xᵢ, xⱼ⟩ with K(xᵢ, xⱼ) in the dual allow the SVM to find non-linear decision boundaries without explicitly mapping the data to a high-dimensional space? Give one example of a kernel that implicitly maps data to an infinite-dimensional space.
